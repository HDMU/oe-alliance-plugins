...
def createTransCodingConfig(encoder):
	if fileExists(getProcPath(encoder ,"bitrate")):
		if HardwareInfo().get_device_name() in ('vusolo2', 'optimussos2plus')
		...
		elif HardwareInfo().get_device_name() in ('gbquad', 'gbquadplus'):
...
class TranscodingSetup(Screen,ConfigListScreen):
	...
	def __init__(self,session):
		...
		from Tools.HardwareInfo import HardwareInfo
		if HardwareInfo().get_device_name() in ('vusolo2', 'gbquad', 'gbquadplus'):
